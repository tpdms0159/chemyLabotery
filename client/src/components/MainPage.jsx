...
function MainPage() {
  ...
  const deleteData =  (cnt) => {
    axios.get(`http://ec2-52-78-9-158.ap-northeast-2.compute.amazonaws.com/find/${name}`)
    .then(res => {
      // 테스트를 처음하는 경우
      ...
      // 테스 경험이 있는 경우
        ...
        if (cnt == 1) {
          ...
          { // '확인' 을 누른 경우 -> 데이터 삭제
            axios
            .delete(`http://ec2-52-78-9-158.ap-northeast-2.compute.amazonaws.com/delete/${name}/${cnt}`,{})
            .then((res) => {
              navigate("/start")
